...
<p align="center">
  ...
  <img alt="ERC" src="https://img.shields.io/badge/ERC-977%20checks%20·%200%20fail-2a9d8f?style=flat-square"/>
...
<h3 align="center">
  A 220 kW-class SiC traction inverter for 500–850 V packs (800 V-class packs whose full-charge voltage stays ≤ 850 V) —<br/>
...
| **L4** hall harness | 5 V per sensor + 3× current signals | `JLEM` (card) ⇄ 3× LEM HC5FW at the phase outputs | 100 k pull-downs (drawn since A.11): an open wire reads 0 V, outside the 0.2–4.8 V window → channel invalid |
...
| 3 | Bleeder **9× TE CRGP (500 V specialty) → 12× generic 22 k 2512 2 W** 6s×2p (66 kΩ; the first cut, 10× 27 k 5s×2p, put 92 % of a 2512's 200 V working on each part) | TE part is TTI-only in volume | 154 V/resistor = 77 % of the 200 V working — any vendor builds it (dfm.md §2 item 3; boards/discharge.tsx RBLD1–12) |
...
300 dpi where the dots decided it) before a single edit. Findings log **F1–F134, all closed**;
...
[`docs/review-A9-disposition.md`](docs/review-A9-disposition.md), and the A.9 schematic rechecks, with their round-11 closure, in
[`docs/review-A10-disposition.md`](docs/review-A10-disposition.md), round twelve (63 findings across two
whole-system reviews) in [`docs/review-A11-disposition.md`](docs/review-A11-disposition.md), and the round-13
rechecks with the production closure (MCU ball map, barrier parts, connector) in
[`docs/review-A12-disposition.md`](docs/review-A12-disposition.md), and round fourteen (three reviews of the A.12
...
| Round 22 | A.21 | 5 (F209–F213) | **three rechecks of the A.20 push — all right, one MINOR of theirs — and the user's question: why is the schematic not furnished for PCB layout?** The reviewers' NOT READY verdicts are bench, target and vendor evidence that needs a built board; the circuit carried no open defect. What was unfurnished was the handoff: every KiCad symbol carried a bare package code and no footprint library shipped, so Update PCB from Schematic could not run for any of the 686 components — **closed: `traction.pretty` (45 patterns copied verbatim from KiCad 10.0.6 with provenance, 18 drawn from the archived datasheets — the EconoDUAL 3 module, UCC14141 DWN-36, AMC1311B DWV, VOW3120, VGT12EEM, Samtec IPL1 ×3, TE 770669-1, the Faratronic can, Bel 2410 fuse, TDK ACT45B, the SQP10 at its real 55.88 mm pitch, onsemi SMA flat-lead, Kyocera 2-pad crystal, the FS26 and ALM2402 lands, LEM HC5FW), `fp-lib-table` and per-board project files in every set and zip, F2 = `traction:<name>` on every on-board symbol (the LEM sensors off-board), the generator refuses an unbound part, the KiCad proof judges footprint links and every pin number on a pad** (F210); the proof and a datasheet pin audit of 31 part groups caught the NCV4276C regulators coded as 3-lead DPAKs (5-lead only, F211), the ALM2402 and TPS55340 thermal pads with no symbol pin (pins 15/17 on ground, F212) and the two-terminal crystal on a 4-pad pattern (F213); the QP §0.2 DUT/FW/VAL baseline and the last 0x0A0F0011 mentions brought to A.20+ / 0x0A0F0013, the README bleeder row, MCU ball count, VR-17, the RFS4 fault-hold row and the dfm fillet rule corrected (F209); **[`docs/layout-handoff.md`](docs/layout-handoff.md)** is the layout engineer's entry point; ₹0/unit |
| Round 21 | A.20 | 1 (F208) | **three rechecks of the A.19 push — all right, nothing on the circuit:** F205–F207 close in all three (the alternates in the emitted CSVs, the firmware ID, the firmware trees identical to A.18; the diversion figures reproduced independently — 9.02 V / 3.84 A / 34.8 A at 24 V, 36.1 A at 35 V, τ 59.9 µs, 0.231 mC; the 4XX discharge 1.47 s at 220 Ω against 3.07 s at 470 Ω); one MINOR documentation defect — gate ㉘ still quoted IR-16 ≥ 0.29 Ω for the 35 V double event where IR-16 says 0.37 Ω (42.6 A against 37.9 A at the cold corner) — confirmed and broader than reported: the same round-18 figure sat in QP-RX-04's own step-3 fixture instruction, and 8.5 V-era figures remained in the QP's post-trip pass criterion (0.6 / 0.3 / 1.5–3.8 W), its Marine cross-reference (5.2 / 8.3 W, the uncapped formula), QP-RX-05's rail figure, VR-33's envelope (4.8 A / 0.28 mC / ≤ 11 V), this gate's trickle and sweep resistances, the card comment and the RSX BOM description (4.8–4.9 A) — every live figure now reads the 7.0 V-class value from the shared model with the old one marked; ₹0/unit |
| Round 20 | A.19 | 3 (F205–F207) | **three rechecks of the A.18 push — all right, nothing on the circuit:** the resolver-origin and high-current findings close in all three; two BOM-metadata errors of ours corrected — the exciter TVS alternate still called the 8.5 V part "same electricals" after the class change (→ the plain SMDJ7.0A, the 8.5 V class excluded from the qualification) and the 4XX discharge row carried the 8XX 470 Ω alternate (→ SQP10AJB-220R; four 470 Ω would take 3.1 s to 60 V against 2 s) — each with an ERC lock; the target checklist's firmware ID brought to 0x0A0F0013; every 8.5 V-era figure re-derived from the shared model at the IR-16 allocations, including the back-drive row that was still evaluated at 0 / 0.29 Ω; ₹0/unit |
| Round 19 | A.18 | 4 (F199–F202) | **three rechecks of the A.17 push — all right, plus one of our own:** the PTC-current row had been evaluated at 24 V while IR-16 allowed 26 V (41.6 A cold at 26 V / 0.05 Ω) — every permitted voltage now, minima at the verifier's own 5 % margin, rounded up (IR-16 0.09 / 0.33 / 0.25 Ω — the 8.5 V-class interim values, superseded within the round); the ≥ 8 A TVS-energy PASS rested on an I⁻² trip-time law no sheet states — an explicit assumption and a WARN closed by the measured clearing waveform (QP-RX-04 step 2 release criterion, VR-16); the resolver ring had anchored its clock on the first completion callback, so an over-budget or constantly-late callback became the reference — **FW-35 rewritten: the origin is the SWG start on the microsecond timer**, every completion judged absolutely, resync from the clock with per-channel slot checks, a synchronized restart on ambiguity; the round-18 window row had printed 1–200 Ω for a criterion error (14–83 Ω at 24 V); the exciter fault model is one shared module, run for the **Marine port at the ship's voltages** — the kit's 12 V rail is the platform's worst sustained-short case (5.2–8.3 W in the TVS by the uncapped 8.5 V-class formula — 2.0–2.5 W at the 7.0 V class after F203/F204), a kit-loom requirement and QP-MA-11 carry it; **one zero-cost part change — TVSEP/TVSEN → SMDJ7.0A-HRA** (the Opus cross-check showed the coupled island cannot hold the 8.5 V class below 148–171 °C at a parked 12 V battery or the Marine rail, because the tripped PTC's trickle scales with V_BR/(V_S − V_BR); the 7.0 V class halves it and stays ≤ 132 °C coupled, keeps every dark condition with ≥ 2.4 V margin, and costs +0.043 Ω on each IR-16 allocation — 0.08 / 0.14 / 0.37 Ω, an OEM statement now needed at 24 V); 290 tests / 2705 checks / 0 failed in three build flavours, 108 failing on the pre-fix ring, 16 mutations caught, TI_FW_ID 0x0A0F0013; ₹0/unit |
| Round 18 | A.17 | 9 (F190–F198) | **three rechecks of the A.16 push — all right, two under-stated:** the diversion Schottky sat on the protected node so its charging loop bypassed the resistor its own model divided by (27–99 A instead of 5 A) — moved to the amplifier node, zero parts; **the generic diode cell numbered the anode as pin 1 on all 35 two-pin diodes** against the parts' pinning tables and KiCad's footprints — one shared cell, ERC-locked on every board; the NCV4276C 40 V output rating withdrawn as reverse-current evidence (release gate + VR-33); the exciter TVS/PTC rows recomputed at their worst corners (the 20 ms bound holds only for a direct short; the 9 J point is ≈ 5.3 J rectangular) and the cross-check's own finding — a sustained short to the normal battery with the ECU asleep leaves 1.5–3.8 W in the TVS after the PTC trips (8.5 V class; 0.9–1.7 W since round 19) — closed by a layout rule, a bench sweep, IR-42 and VR-17; RSX an anti-surge 0.5 W part; firmware: fresh samples no longer aged against the earlier ISR clock, cadence-locked resolver frames with a servicing deadline and self re-acquisition, the chain-latency sign, plus three self-found instances of the same class (task liveness, the FW-15 recovery timer, the PWM-ASC dead-time reference) — 278 tests / 2534 checks / 0 failed in three build flavours, 98 (+2) failing on the pre-fix tree, 16 mutations caught; +₹2/unit (the anti-surge RSX pair) |
...
| Review 12 | A.11 | 12 (F123–F134) | two whole-system reviews (28 + 35 findings): **SPO release rule missed the stored winding energy** (rule (a) now an energy inequality); **resolver exciter drawn with its MFB feedback swapped** (0.18 → 1.85 at 10 kHz); **the V_DC bias "MGJ2D150505SC" never existed** (UCC12050 per channel, V_IOWM 1697 VDC); Hall pull-downs drawn and the LEM drawn by terminal; resolver-pin short to KL30 limited to the MCU's 3 mA; **the LV-entry TVS as described conducted at a 24 V jump start** (24 V-stand-off -VR class bound, load-dump source resistance gated); ALM2402 package, NTC symbol, verifier guard, contract wording; −₹165/unit |
| Review 11 | A.10 | 0 (closure) | schematic-only rechecks of A.10 closed A9-S01, S9-01 and S9-02 and asked for no circuit change; leftover "13.4–18.1 V" wording corrected; ESR03 terminal-temperature limits added to the layout rules |
...
| [`docs/review-A11-disposition.md`](docs/review-A11-disposition.md) · [`.csv`](docs/review-A11-disposition.csv) | Review round 12 (two whole-system reviews, 63 findings): every finding classified, verified and dispositioned |
| [`docs/review-A10-disposition.md`](docs/review-A10-disposition.md) · [`.csv`](docs/review-A10-disposition.csv) | Review round 10 (schematic rechecks of A.9) and its round-11 closure: every finding classified, verified and dispositioned |
...
| [`docs/datasheets/`](docs/datasheets/) | Component datasheet pack (107 PDFs) + extracted parameters |
| [`docs/vendor-requests.md`](docs/vendor-requests.md) | **Vendor requests register** VR-01…VR-33 (round 18): every open vendor input as an exact question with the design condition, the acceptance criterion and the verifier row or gate it closes — HIITIO, NOVOSENSE, TDK, Faratronic, Murata, Bourns, Littelfuse, Diodes, Samtec, NXP, LEM, coldplate |
| [`docs/interface-requirements.md`](docs/interface-requirements.md) | **Vehicle / motor / harness interface requirements** IR-01…IR-42 (round 18): what the inverter assumes about the LV supply, the motor and resolver (data for the safe-state decision, Z_primary ≥ 60 Ω), the harness (exciter fault double event), the HV side, the coolant, the EOL steps and the single-vs-double-fault safety assumptions |
...
| [`docs/layout-handoff.md`](docs/layout-handoff.md) | **Layout handoff** (round 22): the layout engineer's entry point — the KiCad project with every symbol bound to a footprint in the shipped `traction.pretty` (standard patterns copied verbatim from KiCad 10, the module, isolators, transformer, connectors, can, fuse, choke, power resistor and crystal drawn from the archived datasheets — `traction.pretty/MANIFEST.md`), per-board project files, the layer/copper facts, the creepage basis (PD2/OVC II provisional), the placement and routing rules by block, the open decisions with their owners, and the first-article measurements the layout must keep possible |
...
timeline
    ...
    section Done ✅
        ...
        Rev A.11 : System review round 12 answered (63 findings) : F123–F134 · SPO energy rule · resolver MFB · UCC12050 bias · Hall pull-downs
...
> [!NOTE]
> **⚠️ VERIFY-before-layout list (rev A.13)** — ① ~~S32K396 package and ball map~~ **closed in A.13** — the A.12
> freeze had two supply balls wrong (H5 = V15, J7 = V25; round 14); the map is now cross-checked ball by ball
> against NXP's GEN3 net report and every signal sits on a netlist-confirmed ball, the KiCad symbol numbers pins
> by ball ([`docs/mcu-pin-manifest.md`](docs/mcu-pin-manifest.md)); FS26 48 pins verified, connector bound · ② **SiC double-
...
> ⑧ ~~4XX can MPN~~ **bound in A.12** (Faratronic C3D1U506KFAA382; ripple rating checked at PO) · ⑨ LEM lead time — **order T2 parts at kickoff** · ⑩ LEM sign-off
> on the phase-busbar insulating sleeve (HC5FW/SP1 is the reduced-insulation variant, N7) · ⑪ ~~TLP152
> barrier rating~~ **closed in A.12**: both optos are Vishay VOW3120 (V_IORM 1414 Vpk, DIN EN 60747-5-5, CPG ≥ 10 mm; N8) ·
> ⑫ HIL then dyno: contactor opening under full regen. The FW-06 chain is ≤ 15.6 µs to the ASC
> request plus ≤ 7.6 µs ASC entry, a link peak of ≤ 906 V on 8XX (N9, RR06; UCC14141-Q1 low end, A.12).
...
> ⑯ ~~Confirm PTC25/PTC26 reach an eFlexPWM FAULT input~~ **closed in A.12**: R14/PTC25 = PWM_1_FAULT[2],
> P15/PTC26 = PWM_1_FAULT[0] — the same eFlexPWM_1 instance as the six outputs; every VDD_HV_A/B ball on V5A
> (`docs/mcu-pin-manifest.md`).
> ⑰ NOVOSENSE statement on DESAT during ASC with EN low (§8.12 marks it irrelevant); this
> decides the FS1B-ASC residual.
> ⑱ VOW3120 100 °C ambient vs the power-board interior; LED drive and timing at corners on both optos.
...
> ㉔ ~~Barrier working-voltage statements for the parts that only quote a hipot~~ **closed at BOM level in A.12**:
> PSASC/PSQD are TI UCC14141-Q1 (reinforced, V_IORM 1414 Vpk / V_IOWM 1000 Vrms, IEC 60747-17) and the Y-caps
...
> ㉘ Terminal-fault bench (round 14, F04/F18; widened rounds 15–16; re-scoped round 17; two release gates restored
> round 18): each resolver excitation line and each motor-temperature line to 24 V/60 s and 35 V/400 ms with
> VEXD/the MCU on, off and cranking, positive and (where the OEM allocates it) negative. Characterisation for the
> sheet-bounded single fault (fault current ≥ 8 A — the ISO 16750-2 direct short: TVS energy ≤ 1.7 J against the
> exciter TVS's 5.3 J rectangular-equivalent — SMDJ7.0A-HRA since round 19 (F203) — PTC current ≤ 40 A at the IR-16 harness minimum), the PTC hold (IR-13)
> and the 35 V double event (IR-16 ≥ 0.37 Ω at 35 V since round 19 — round 18's 0.29 Ω was the 8.5 V-class figure and gives 42.6 A
> cold against the PTC's 40 A, 37.9 A at 0.37 Ω; round 21 F208 / IR-33). RELEASE GATES since round 18 (F191–F194): (a) QP-RX-05 — the
> reverse rail current into ULDOEX with its input at 0 V, cranking and inhibited: the diversion diodes DEXP/DEXN
> now sit on the amplifier node so the charging loop passes RSX (≈ 3.9 A / 60 µs with the 7.0 V clamp, judged by I²t), and the NCV4276C's
> 40 V output rating covers the node voltage only (VR-33); (b) QP-RX-04 step 2b — a sustained short through
> 0–100 Ω external at 12.6/14.4/16/24 V with the ECU asleep, where below 8 A the PTC's trip time is a typical curve
> and the tripped PTC's trickle leaves 0.9–1.7 W in the TVS at 12.6–16 V, 1.5–3.8 W with the 8.5 V class of round 18 (dfm.md: TVS copper island with the PTC thermally coupled
> on it; IR-42; VR-17). RSX is an anti-surge 0.5 W part (the powered negative fault). Since round 19 (F199/F200) the
> ≥ 8 A clearing measurement (trip ≤ 20 ms, ∫v·i ≤ 5.3 J at 0 / 0.14 / 0.5 / 1.33 Ω, 16 / 24 / 26 V) is a release criterion too —
> the sheet bounds only the 8 A point — and the Marine kit runs the same sweep at the ship's voltages (QP-MA-11: its
> 12 V rail is the platform's worst case). Measured for the record:
