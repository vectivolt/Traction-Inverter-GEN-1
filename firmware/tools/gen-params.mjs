...
//
// Source of every number: docs/firmware-contract.md rev A.12 (section named per row) and the SKU
...
// ---------------- FW-10 excitation chain (round 16, A14-N01: amplitude planes) ----------------
// SWG -> MFB (13 k in, 28 k feedback since rev A.15, 10 k, 1.5 nF, 220 pF) -> ALM2402 bridge (x 2) -> RSX 2.2 ohm
// per line -> monitor tap (the protected node) -> PTC per line (1.3 ohm cold, up to 5 ohm for an hour after a
// trip) -> resolver primary (70 ohm screening assumption; the selected resolver decides, gate 25).
const MFB = { r1: 13e3, r2: 28e3, r3: 10e3, c1: 1.5e-9, c2: 220e-12 };
const mfbGain = (f) => {
  const w = 2 * Math.PI * f, a = 1 - w * w * MFB.c1 * MFB.c2 * MFB.r2 * MFB.r3;
  return (MFB.r2 / MFB.r1) / Math.hypot(a, w * MFB.c2 * (MFB.r2 + MFB.r3 + MFB.r2 * MFB.r3 / MFB.r1));
};
const RSLV = { pri: 70, rsx: 2.2, ptcCold: 1.3, ptcTrip: 5.0 };
const loadCold = RSLV.pri + 2 * RSLV.ptcCold;
...
const COMMON = [
  ...
  ["asc_entry_max_us", 7.56, "§4c hardware ASC entry <= 7.56 µs (design-verify, Safety A.8: ASC entry, latch set -> LS gates on)"],
  ...
  ["vsup_amux_ratio", 14, "LV supervision (round 17): VSUP on the FS26 AMUX, divider option 1 (DS Tables 56, 130)"],
  ["vsup_valid_min_v", 4.2, "LV supervision: AMUX divider 14 input range 4.2–36 V (DS Table 131); below = no reading"],
  ["vsup_ov_v", 20.0, "LV supervision: the FS26 VSUP_OV threshold (19.3–20.7 V, DS Table 9) — VSUPOV"],
  ["vsup_ov_hyst_v", 0.5, "LV supervision: VSUPOV ends below vsup_ov_v - this"],
  ...
  ["rslv_floor_vpp", 6.5, "gate 25: resolver minimum excitation, AT THE WINDING"],
  ["exc_gain", 2 * mfbGain(10e3), "SWG -> amplifier differential: MFB |H(10 kHz)| (28 k, rev A.15) x 2 (ALM2402 bridge)"],
  ["exc_amp_per_mon", (loadCold + 2 * RSLV.rsx) / loadCold, "amplifier / monitor plane: RSX 2.2 ohm per line before the tap, PTC 1.3 ohm cold per line + 70 ohm primary after it"],
  ["swg_maxapp_min_vpp", 1.884, "S32K39 DS Table 40: MAXAPP min, the SWG low corner"],
  ["exc_slew_max_vpp", 4 * 0.13e6 / (2 * Math.PI * 10e3), "ALM2402 ~0.13 V/us at -40 degC: 2.07 V pk per output at 10 kHz (x 4 = differential V pp)"],
];
...
const CAL = [
  ...
  ["cal_dcl_kp_nm_v", 0.5, 0.05, 5, "f", "FW-08 DC-link trim (RUN only, battery proven): regen taken back per V above vdc_max_v"],
  ["cal_dcl_ki_nm_vs", 20, 1, 200, "f", "FW-08 DC-link trim integral"],
  ["cal_dcl_tmax_nm", 50, 5, 200, "f", "FW-08 DC-link trim authority: the most regen it takes back"],
  ...
  ["cal_desat_en_hold_us", 60, 55, 250, "u", "A12-R05: no software MCU_GATE_EN drop within this time of a FLT (latch path 22–53 µs + margin)"],
  ["cal_vdyn_reserve_frac", 0.05, 0, 0.2, "f", "F23: voltage kept free for current-loop dynamics (torque->current feasibility)"],
  ["cal_vsup_ld_ms", 500, 400, 1000, "u", "LV supervision (round 17): tolerated time above cal_vsup_jump_max_v — IR-03, the ISO 16750-2 test-B let-through pulse (up to 35 V, td <= 400 ms) + margin; floor = the pulse, ceiling 2.5x it (the FS26's limited HV-extended period is VR-29); longer = sustained"],
  ["cal_vsup_jump_ms", 65000, 60000, 120000, "u", "LV supervision: tolerated VSUPOV event at or below cal_vsup_jump_max_v — IR-02, the ISO 16750-2 24 V jump start (60 s) + margin; ceiling 2 min (IR-02: the hardware rows are sized for one minute); longer = sustained"],
  ["cal_vsup_jump_max_v", 27.0, 24.5, 30, "f", "LV supervision: jump-start band ceiling — the 2023 edition's 26 V jump start at the AMUX's highest reading (x1.015 + 0.1 V = 26.5 V, DS Table 131) + margin; floor keeps IR-02's 24 V (24.46 V) inside, ceiling keeps a test-B plateau (VSUP 34.6 V, >= 34.0 V read) above"],
  ["cal_asc_release_ns", 1500, 1070, 5000, "u", "FW-06a step 3 (round 17): the ASC pins release <= 1.07 µs after ASC_CLR (design-verify Safety A.8: VOW3120 tpHL 0.5 + DASCR 0.08 + NSI6611 tASC_f 0.48 µs + 11 ns of logic) + margin; the first HS pulse waits this + the SKU dead time (the LS turn-off) from the clear's falling edge; never below the release"],
  ["cal_isns_act_min_a", 20, 5, 100, "f", "F24: a phase whose reference reaches this must show current"],
  ["cal_isns_act_frac", 0.2, 0.05, 0.5, "f", "F24: measured phase current vs its reference (stuck channel below)"],
  ["cal_isns_act_debounce", 20, 4, 100, "u8", "F24: consecutive applicable samples below the fraction => stuck channel"],
  ["cal_rslv_hold_us", 500, 250, 800, "u", "A14-R01: newest coherent resolver frame (block start) may be this old, else the angle is withdrawn. The hold may add at most the angle error the observer already carries at the acceleration envelope (alpha/wn^2): extrapolating adds e_w*t + alpha*t^2/2 with the observer's peak speed lag e_w = alpha/(e*wn), equal at t = 1.09/wn = 580 us (wn = 2 pi 300 Hz), whatever alpha; 500 us adds 0.26 deg el at alpha = 2e4 rad/s^2. Floor: 2 carrier periods + IRQ jitter (a normal empty read); ceiling: the observer's 8-block re-acquisition gap"],
  ...
  ["cal_sd_irq_lat_max_us", 30, 5, 45, "u", "A16-R02 (round 18): a resolver block's first SDADC completion interrupt is serviced within this of the block's end, else the frame is not published and the ring re-acquires (a later one cannot tell a delay from a lap of the 4-slot ring). The priority-1 handler waits at most for the fault ISR (<= 10 us budget), a PRIMASK section and its two sibling handlers (~1 us each): ~13 us, x2.3 margin; floor 5 us above entry + the siblings; ceiling 45 us < half the 100 us carrier period. Round 19: the block's end is on the SWG-start cadence, so the SDADC's own output latency (decimation filter, FIFO watermark, eDMA) counts in it too — T-40 measures the sum from the carrier boundary"],
  ["cal_swg_start_lat_us", 2, 0, 20, "u", "A17-R01 (round 19): the resolver cadence's origin is the SWG start: hal_swg_start() anchors the ring at the later of two time reads around the enable, uncertain by the bracket (+1 us of timer resolution) plus this — the SGEN's start to its first period (the RM) plus the TRGMUX/SDADC trigger latency: sub-us each (a few bus/ADC clocks), 2 us allowed. It widens every completion window both ways and bounds every stamp's error (2 us + 1 = 3 us = 0.7 deg el at 10 000 rpm, 4 pole pairs); ceiling 20 us keeps the re-sync window (P/2 + 2 x uncertainty) inside the 100 us period. HIL-measured: T-42"],
  ["cal_rslv_restart_max", 3, 0, 10, "u8", "A17-R01 (round 19): synchronized producer restarts per key cycle (a DMA out of phase with the carrier or lost samples: the rings re-armed, the SWG restarted and re-anchored, one DTC_RSLV_REACQUIRED each); then the resolver stays invalid for the key cycle (FW-28). 3: a transient and its repeat recover, a persistent fault stops the retries after three (the FW-28 age-out acts from the first loss); 0 = never restart"],
];
...
const header = (s) => {
  ...
  return `/* GENERATED by tools/gen-params.mjs — do not edit. Source: docs/firmware-contract.md rev A.12
