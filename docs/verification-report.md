# Design Verification Report — rev A.14 (2026-09-25)
...
3. **Numeric verification** (this report, `design-verify.mjs`): **132 PASS · 24 WARN · 0 FAIL** (+18 info)
...
## Findings log (F1–F36 rev A.3 campaign · F37–F46 rev A.4 · F47–F51 rev A.4.1 · F52–F57 rev A.4.2 · F58–F59 rev A.4.3 · F60–F62 rev A.5 docs audit · F63–F76 rev A.6 external review round 6 · F77–F89 rev A.7 review round 7 · F90–F97 rev A.8 review round 8 · F98–F105 its cross-check · F106–F113 rev A.9 review round 9 · F114–F119 its cross-check · F120–F122 rev A.10 schematic rechecks · F123–F134 rev A.11 system review · F135–F142 rev A.12 rechecks, pin freeze and production closure · F143–F158 rev A.13 round 14 · F159–F164 rev A.14 round 15 (three rechecks of a8c75eb) — all fixed; review cross-reference in [`review-A6-disposition.md`](review-A6-disposition.md), [`review-A7-disposition.md`](review-A7-disposition.md) and [`review-A8-disposition.md`](review-A8-disposition.md))
...
| Exciter terminal fault — clamp at the protected node during the PTC trip window (PTC in the path, A13-R01) | 11 V at 24 V (15.3 A) · 11.5 V at 35 V (27.7 A ≤ 40 A PTC I_max) | ≤ 12.8 V while VEXD is up (12.1 V rail + a diode) · 18 V output abs max | ✅ PASS | 90% of limit · F04: the amplifier's regulated 12.1 V rail does not protect an output pin forced by the harness (ALM2402 output abs max 18 V; its output diodes conduct reverse current only as pulses, DS §8.3.6 — the clamp below the rail avoids that entirely). SMCJ8.5CA V_BR 9.44–10.4 V clears the 4–8 V excitation swing; the MF-MSMF020 (0.2 A hold, 30 V, AEC-Q200) trips in 0.06 s at 6 A and faster at these 30–70× overloads, then holds the TVS at ≈ 0.3–0.8 W. Line-to-ground shorts are the ALM2402's own current limit (≈ 750 mA). BENCH (gate ㉘): both lines, MCU on/off, 24 V/60 s and 35 V/400 ms |
| Exciter TVS energy during the PTC trip window (PTC in the path) | 3.4 J at 24 V (168 W × ≤ 20 ms) · 3.2 J at 35 V (317 W × ≤ 10 ms) | ≈ 8 J (SMC 1.5 kW at 1 ms, 550 W at 10 ms, 6.5 W steady) | ✅ PASS | 42% of limit · round 15 (A13-R01): the round-14 drawing had the TVS on the connector node, where a 24/35 V fault fed it 25/46 A (287/555 W) with nothing to interrupt — the PTC is now between the connector and the clamp. Trip time from the Bourns 8 A point (0.02 s); the bench (gate ㉘) measures it. If slower than 100 ms, the 3 kW 5.0SMDJ8.5CA fits the same pad |
| Exciter back-drive with VEXD absent (reverse diode through RSX 2.2 Ω until CLDE charges) | 4.9 A for ≈ 48 µs (0.17 mJ in the diode), rail then ≈ 10.8 V | diode: pulsed use only (DS §8.3.6); rail ≤ 18 V abs max | ✅ PASS | 60% of limit · A13-R02: with the rail down the 'no back-drive' argument does not hold — RSX bounds the pulse and the pumped rail sits under the ALM2402 18 V abs max until the PTC trips. Bench gate ㉘: VEXD off / cranking / on, reverse rail current into ULDOEX measured (NCV4276C output-above-input behaviour is not specified) |
| Exciter negative terminal fault (line pulled below ground; bidirectional clamp −9.4…−10.4 V) | lower reverse diode ≈ 4.4 A through RSX until the PTC trips (≤ 20 ms) | ALM2402 output −0.3 V abs; diode pulse only | 🟡 WARN | whether a negative fault exists on this connector is an OEM allocation (reverse-battery, inductive kick); RSX bounds it — gate ㉘ includes the negative case where allocated |
| Excitation amplitude at the resolver through RSX + PTC (70 Ω primary) | 7 V pp cold (6.39 V pp for an hour after a trip) | ≥ 6.5 V pp (gate ㉕) | ✅ PASS | 93% of limit · round 15: the 6.5 V pp floor is evaluated at the resolver, not at the amplifier; the monitor REXM now taps the protected node so the FW-10 window and the SWG trim see the same point; post-trip the FW-10 window flags the line (intended) |
...
| Check | Value | Limit | Verdict | Margin note |
|---|---|---|---|---|
| Stuck-ON QDIS with the battery connected, 8XX | 0.45 A · 96 W per resistor (10 W rated) | resistor must fail open without flame (gate ㉖) | 🟡 WARN | F09: the software timeout is bypassed by a shorted switch; the firmware detects the unexpected discharge at the next contactor opening (latched DTC, no re-energisation) — the benign-failure test of the SQP10 at this power is the release evidence |
| Stuck-ON QDIS with the battery connected, 4XX | 0.57 A · 71 W per resistor (10 W rated) | resistor must fail open without flame (gate ㉖) | 🟡 WARN | F09: the software timeout is bypassed by a shorted switch; the firmware detects the unexpected discharge at the next contactor opening (latched DTC, no re-energisation) — the benign-failure test of the SQP10 at this power is the release evidence |
