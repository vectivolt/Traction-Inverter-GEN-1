# Simulation report (rev A.6 · generated 2026-09-22)
...
| S1 | Gate-power STARTUP, A.6: 2.2 k / 47 µF (≈37 µF eff) | KL30 9 V: typ 192 ms (1 burst) · worst 295 ms (1 burst) · KL30 14 V worst 75 ms (1 burst) | one-burst start at every corner, KL30 9–16 V | ✅ PASS | worst = UVLO 7.5/7.1 V, 3.5 mA IC, 45 nC FET, CS 0.9 V, Lp −20 %, 3 mA driver current; 22 µF (18 µF eff) still fails this corner |
| S1 | Flyback bank load, SiC @10 kHz (1.09 µC) | 1.33 W demand | 2.31 W at worst parts (CS 0.9 V, Lp −20 %, 100 % limit) | ✅ PASS | same rails, same transformer for every SKU |
| S1 | Flyback bank load, IGBT @5 kHz (4.36 µC, unscaled, RT 8.2 k) | 2.01 W demand | 2.81 W at worst parts (CS 0.9 V, Lp −20 %, 100 % limit) | ✅ PASS | same rails, same transformer for every SKU |
...
| S4 | Tj end of 30 s peak — 8XX IGBT (340 A, 850 V, 5 kHz) | 127 °C (from 98 °C continuous) | 150 °C Tvjop | ✅ PASS | 560 W/switch peak, 250 W continuous (hottest die); coldplate 0.045 K/W assumed |
| S4 | Tj end of 30 s peak — 4XX IGBT (400 A, 500 V, 5 kHz) | 121 °C (from 98 °C continuous) | 150 °C Tvjop | ✅ PASS | 496 W/switch peak, 253 W continuous (hottest die); coldplate 0.045 K/W assumed |
...
| S7 | Gate peak current on/off, IGBT (1.0/1.0 Ω) | 5.6 / 10 A | 10 A driver | ✅ PASS | sink at the driver's own limit |
...
| S9 | DESAT reaction, IGBT 82 pF | 4.46 µs (detect 2.98 + soft-off 1.48; 8.9 µs at 100 mA) | 6 µs @800 V (≈5 µs at 850 V/15.6 V) | ✅ PASS | 150 pF gave 4.5 µs detection alone (F03). Global DRV_EN drop is held 12–40 µs past this (RC delay) so it cannot cut the soft-off short |
| S10 | ASC hold-up after TOTAL LV loss (gate reservoirs) | 3.2 ms typ · 1.1 ms worst | - | ⚠️ WARN | and the ASC command path collapses within ≈1 ms: sustained ASC REQUIRES KL30 (FS26 GPIO1 holds the flybacks). With LV dead the bridge is three-phase-open — energy-safe only if the motor's E_LL,pk at n_max (cold magnets) < the 1000 V cap rating; otherwise fit the HV-fed backup-bias option (motor-dependent, see firmware contract) |

**24 PASS · 4 WARN · 0 FAIL**
