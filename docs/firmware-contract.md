...
- **FW-05** Phase overcurrent: ADC channel thresholds with hardware compare (S32K396 ADC
  watchdog / BCTU → eFlexPWM_1 FAULT input; the S32K39 routing of the ADC watchdog to a PWM fault is a HIL item — see `firmware/docs/traceability.md`) at **1.25 × √2 × the SKU peak rms current, in instantaneous
...
- **FW-07** V_DC plausibility: |VDC1 − VDC2| > 5 % ⇒ fault; `VOFS` (the receivers' shared
  ...
  state, not a dead bus. **Both channels are also invalid whenever V5GD (PTD27 since the A.12 ball map — PTB5 has no ADC on the 289-MAPBGA; V5GD/2) is outside
...
- **FW-08** Regeneration with the battery path lost (contactor opens; a BMS charge limit that
  ...
  leaving the pack. Below n_x: zero torque under current control while winding current remains (FW-06 LS-ASC
  as the backstop), then SPO; at or above n_x, or at unknown speed: LS-ASC. Torque permission is withdrawn in
  the same cycle. A zero-torque opening below n_x with no winding current is a normal disarm, not a fault.
...
| FW-11 | CAN torque command | counter + CRC, ≤ 20 ms staleness ⇒ ramp to zero torque (not hold last value) — round 15: a stale command also makes the contactor feedback invalid, so while armed the §6 battery-lost row supersedes the ramp; the ramp still applies to a command lost with the battery path proven (e.g. HVIL open); BMS limit timeout ⇒ zero regen |
