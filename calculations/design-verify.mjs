...
// ============ 7b. ROUND 12 — system review of 4544715 (docs/review-A11-disposition.md) ============
{
  ...
  // discharge stuck-on, SC extinction budget, firmware DESAT hold vs the hardware RC ----
  {
    ...
    const vbr = [9.44, 10.4], rdyn = (14.4 - 10.4) / 104.2, rPath = 1.3 + 0.5;   // MF-MSMF020 cold ≈ 1.3 Ω typ (0.4 min → 27 A, clamp 11.4 V) + harness 0.5 Ω
    const iF = (v) => (v - vbr[1]) / (rPath + rdyn), vCl = (v) => vbr[1] + iF(v) * rdyn;
    judge("Sensing A.13", "Exciter terminal fault — clamp seen by the ALM2402 output during the PTC trip window", `${f(vCl(24), 1)} V at 24 V (${f(iF(24), 1)} A) · ${f(vCl(35), 1)} V at 35 V (${f(iF(35), 1)} A)`, "≤ 12.8 V (12.1 V rail + a diode: no back-drive) · 18 V output abs max", vCl(35) / 12.8, 0.95,
    ...
    const eTvs24 = vCl(24) * iF(24) * 0.06, eTvs35 = vCl(35) * iF(35) * 0.01, capTvs = 250 * 0.06;   // 24 V: 0.06 s trip at ≈ 6 A (sheet point); 35 V: < 10 ms at > 60×; SMC ≈ 250 W at 60 ms (Fig. 2 extrapolated from 1.5 kW/1 ms, 550 W/10 ms)
    judge("Sensing A.13", "Exciter TVS energy during the PTC trip window", `${f(eTvs24, 1)} J at 24 V (${f(vCl(24) * iF(24), 0)} W × 60 ms) · ${f(eTvs35, 1)} J at 35 V (${f(vCl(35) * iF(35), 0)} W × ≤ 10 ms)`, `≈ ${f(capTvs, 0)} J (SMC 1.5 kW at 1 ms, 550 W at 10 ms, 6.5 W steady)`, Math.max(eTvs24, eTvs35) / capTvs, 0.6,
      "the MF-MSMF020 trip time at 6 A is the Bourns sheet point (0.06 s); the 35 V case is bounded by the < 10 ms trip at > 60× I_hold. If the bench trip is slower than 100 ms, the 3 kW 5.0SMDJ8.5CA fits the same pad (gate ㉘)");
    add("Sensing A.13", "Excitation amplitude through the PTCs", "2 × 1.3 Ω typ cold in series with a ≈ 70 Ω resolver primary: −4 % (2 × 6 Ω for an hour after a trip: −15 %)", "≥ 6.5 V pp at the resolver (gate ㉕)", "INFO", "7.7 V pp designed → ≈ 7.4 V pp; after a fault event the FW-10 amplitude window flags the line until the PTC cools — the intended behaviour; EOL calibrates with the harness fitted");
