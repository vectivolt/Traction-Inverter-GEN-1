...
import * as EX from "./exciter-fault.mjs";   // round 19: the exciter terminal-fault model shared with marine/calc/marine-verify.mjs
...
// ---------- rev A.4 corrections (external design review F37–F46) ----------
{
  ...
  // boot hold. Part: ROHM ESR03EZPF1001 until round 17 (0.33 W at 70 °C → 0.27 W at 85 °C, overload ≈ 1.3 W); since round 17
  // (F179) the 1206 ESR18EZPF1001: 0.5 W at 70 °C → 0.41 W at 85 °C on the Fig. 1 line, overload 2 × U_R for 5 s ≈ 2.0 W
  // (round 22: this row's limit still named the ESR03 — QP §15 item 6).
  {
    ...
    const [p16, p24, p35] = [16, 24, 35].map(pAt), pCont = 0.5 * (155 - 85) / (155 - 70);
    judge("Safety A.9", "RFS4 with FAULT_OUT shorted to KL30 and FS1B asserted (22 mA limit end)", `${f(p16, 3)} / ${f(p24, 3)} / ${f(p35, 3)} W at 16 / 24 / 35 V`,
      `${f(pCont, 2)} W continuous at 85 °C (ESR18EZPF1001 1206, 0.5 W at 70 °C, since round 17 — the ESR03's 0.27 W until round 22); ≈ 2.0 W for 5 s overload`, p16 / pCont, 0.9,
...
// ============ 7b. ROUND 12 — system review of 4544715 (docs/review-A11-disposition.md) ============
{
  ...
  // discharge stuck-on, SC extinction budget, firmware DESAT hold vs the hardware RC ----
  {
    ...
    // Round 19: the model lives in calculations/exciter-fault.mjs (shared with the Marine port); names kept for the rows below.
    const { VBR: vbr, VBR_COLD: vbrCold, RDYN: rdyn, RDYN8: rdyn8, R_PTC: rPtc, R_HARN: rHarn, PD_PTC: pdPtc, iF, vCl, pTrickle, ECAP25: eCap25, ECAP85: eCap85, rExt8, tTrip, RTH_JA: rthJA, pSs, sweep, win } = EX;
    const tab = (v, rs) => EX.tab(v, rs, f);
    ...
    const e8 = 8 * (vbr[1] + 8 * rdyn) * EX.T_TRIP_8A;   // the sheet's one maximum: 8 A for 20 ms
    const eAssumed = (i) => (vbr[1] + i * rdyn) * i * Math.min(EX.T_TRIP_8A, EX.MAX_TYP_8A * tTrip(i));   // ASSUMED (screening): the 8 A max/typ ratio carried above 8 A
    add("Sensing A.15", "Exciter TVS energy — single fault (line shorted to KL30, ≤ 26 V) at fault currents ≥ 8 A: the region the ISO 16750-2 direct short falls in", `${f(e8, 2)} J at the sheet's 8 A / 20 ms maximum · above 8 A the sheet gives NO maximum — ASSUMED (screening): the 8 A max/typ ratio (${f(EX.MAX_TYP_8A, 1)}×) carried up the typical curve gives ${f(eAssumed(20), 2)} J at 20 A, ${f(eAssumed(40), 2)} J at 40 A · if the current persisted the full 20 ms (no basis, the reviewers' sensitivity): 4.3 J at 20 A, 8.9 J at 40 A · the region is external ≤ ${f(rExt8(24), 2)} Ω at 24 V, ≤ ${f(rExt8(16), 2)} Ω at 16 V, ≤ ${f(rExt8(26), 2)} Ω at 26 V`, `${f(eCap85, 1)} J — the ${EX.TVS} 10 ms point, rectangular-equivalent at T_J(init) 85 °C (${f(eCap25, 1)} J at 25 °C); ≈ 7.1 J at 20 ms on the plain SMDJ sheet's Z_th curve (Fig. 6, typical)`, "WARN",
      "round 19 (A17-R02 md/html, all three rechecks; F200): round 18 called this region PASS on an I⁻² trip-time law that no sheet states — the sheet's own typical curve (p.8 C, to 15 A) has t ∝ I^−1.63…−1.80 above 2 A, so I⁻² is contradicted, not supported; only the 8 A / 20 ms point is a maximum. What the energy row needs is I·t non-increasing above 8 A (n ≥ 1: typical n 1.63–1.80; adiabatic physics gives charge q ≤ H/(I0·R0), falling as 1/I0) — the transfer of the maximum curve to a stiff source is ASSUMED. The region is a RELEASE CRITERION of QP-RX-04 step 2 (measured ∫v·i ≤ 0.5·E_cap(t), trip ≤ 20 ms, I_peak ≤ 40 A, at 0 / 0.14 / 0.5 / 1.33 Ω external, −40 / 23 / 85 °C and post-trip; ≥ 5 parts × 2 lots); VR-16 re-scoped (maximum time AND charge to trip from a stiff source, the test circuit behind the 20 ms point, R at −40 °C, I_max at −40 °C, tripped P_d vs voltage). Margin ≥ 2.7× everywhere on the typical curve; a monotone-only bound (20 ms for every I ≥ 8 A) would cover only ≤ 32.6 A (7.1 J on Z_th)");    // --- unbounded region: the typical /33X time-to-trip curve (sheet p.8 curve C, digitised: xcheck18 §3) ---
    ...
    add("Sensing A.17", "Sustained exciter short to the NORMAL battery (12.6–16 V) with the ECU asleep — post-trip trickle and the never-trip region", `TVS after a trip: ${f(pTrickle(12.6), 2)} W at 12.6 V · ${f(pTrickle(14.4), 2)} W at 14.4 V · ${f(pTrickle(16), 2)} W at 16 V (V_BR,max; ${f(pTrickle(12.6, vbr[0]), 2)} W at V_BR,min) · never-trip region (external > ${f((12.6 - vbr[1]) / 0.4 - rPtc, 0)} Ω at 12.6 V, > ${f((16 - vbr[1]) / 0.4 - rPtc, 0)} Ω at 16 V): up to ${f(vbr[1] * 0.4, 1)} W at 23 °C, ${f(vbr[1] * 0.14, 1)} W at 85 °C (I_trip ≈ 2 × hold)`, `steady state ${f(pSs(23, rthJA), 2)} W at 23 °C / ${f(pSs(85, rthJA), 2)} W at 85 °C (75 K/W) — ≈ ${f(pSs(23, 40), 1)} / ${f(pSs(85, 40), 1)} W on the 40 K/W island`, "WARN",
      `round 18 (xcheck18 §3, NEW — no row covered it; F193): the largest gap is at the normal battery voltage, not at 24 V: with the amplifier off (V5A = 0 pulls the ALM2402 SDN low; LPOFF; or after its OTF) nothing sinks the trickle and the TVS junction reaches ${f(23 + pTrickle(12.6) * rthJA, 0)} °C at 12.6 V on the minimum pads (${f(23 + pTrickle(12.6) * 40, 0)} °C on the island). The awake amplifier sinks ≤ 0.36–0.70 A through RSX, which keeps the TVS dark for external ≳ 5 Ω. Round 19 (F203, xcheck19 §1): the trickle is capped at V_BR·I_trip (a tripped PPTC passes at most its trip current — round 18's 5.2 W at 12 V was the uncapped formula) and TVSEP/TVSEN are SMDJ7.0A-HRA: 2.0 W at 12.0 V and 1.7 W at 12.6 V uncapped (the 8.5A: 4.2 / 3.8 W), and on the coupled island — the tripped PTC is a thermostat, the TVS heat it receives replaces its own I·V_P — the junction stays ≤ ${f(EX.coupledTj(11.4, 85, 125), 0)} °C at 11.4 V / 85 °C ambient / T_t 125 °C (next row; the 8.5A: ${f(EX.coupledTj(11.4, 85, 125, 10.4), 0)} °C). The never-trip region is closed by the coupling for every class (I = 2·I_hold(T_island) → ≤ 2.7 W, T_J ≤ 107 °C). Still a WARN: the coupling parameters are assumed until QP-RX-04 step 2b measures the pair; IR-42; VR-17 (narrowed: confirm the plain sheet's Z_th Fig. 6 for the HRA part). The 60 s ISO 16750-2 short at 16 V passes on the island (${f(23 + pTrickle(16) * 40, 0)} °C from 23 °C)`);
    const cT = (vs, ta, tt, vb) => f(EX.coupledTj(vs, ta, tt, vb), 0);
    add("Sensing A.17", "Coupled island — TVS junction with the tripped PTC as a thermostat on the same copper (model with assumed transfers: TVS↔PTC 40 K/W, TVS 55 K/W, PTC 90 K/W — xcheck19 §1h): SMDJ7.0A-HRA vs the round-18 SMDJ8.5A-HRA", `T_t 110 °C: ${[11.4, 12.0, 12.6].map((v) => `${v} V ${cT(v, 23, 110, vbr[1])} / ${cT(v, 85, 110, vbr[1])} °C`).join(" · ")} (T_A 23 / 85) · T_t 125 °C: ${[11.4, 12.0, 12.6].map((v) => `${v} V ${cT(v, 23, 125, vbr[1])} / ${cT(v, 85, 125, vbr[1])} °C`).join(" · ")} — the 8.5A at T_t 125: ${[11.4, 12.0, 12.6].map((v) => `${cT(v, 85, 125, 10.4)} °C`).join(" / ")} (85 °C ambient)`, "T_J ≤ 150 °C (SMDJ-HRA); the PTC switching temperature T_t is 110–125 °C (not on the sheet: VR-16 f)", "INFO",
      "round 19 (F203): the reason for the class change — the coupling benefit scales with V_P/V_BR (V_P = V_S − V_BR), which only the class sets; with the 8.5A the low-V_P case (a parked battery at 12.0–12.4 V, the Marine kit rail at 11.4–12 V) stays at 150–170 °C however good the island. The layout rule must specify the PAIR (the TVS-to-PTC transfer and the PTC's own path), not a TVS-alone R_thJA: a lower island resistance alone RAISES the coupled T_J (T_J − T_A → (T_t − T_A)·(1 + R_thJL·V_BR/(R_isl·V_S)) as R_P → 0). QP-RX-04 step 2b measures the pair's T_J");
    // --- PTC current against I_max 40 A at EVERY permitted fault voltage (round 19, A17-R01 md: the round-18 row was hard-coded at 24 V while IR-16 allowed 26 V) ---
    const IR16 = { 16: 0.08, 24: 0.08, 26: 0.14, 35: 0.37 };   // IR-16 minima (round 19, SMDJ7.0A-HRA): the cold-corner current at the verifier's own 5 % margin (38 A), rounded UP, never down
    const rMin = Object.fromEntries([16, 24, 26, 35].map((v) => [v, EX.rExtMin(v)]));
    const iAt = (v) => EX.iCold(v, IR16[v]);
    judge("Sensing A.15", "Exciter PTC current vs I_max 40 A — single fault at every permitted voltage (16 V nominal, 24 V, the ISO 16750-2:2023 26 V jump start) with the IR-16 minimum harness (0.08 Ω at ≤ 24 V, 0.14 Ω at 26 V)", `${[16, 24, 26].map((v) => `${v} V: ${f(iAt(v), 1)} A`).join(" · ")} cold (V_BR ${f(vbrCold, 2)} V, r_dyn ${f(rdyn8 * 1e3, 1)} mΩ) · at a true 0 Ω: ${f(EX.iCold(24, 0), 1)} A at 24 V, ${f(EX.iCold(26, 0), 1)} A at 26 V (37 A at the V_BR,max corner round 17 used)`, "40 A (MF-MSMF020/33X I_max; trip-cycle life tested at V_max/I_max) — judged at 95 %: the IR-16 minima keep the cold-corner current ≤ 38 A", Math.max(...[16, 24, 26].map(iAt)) / 40, 0.95,
      `round 19 (A17-R01 md, F199): the round-18 row evaluated 24 V only while IR-16 allowed 0.05 Ω up to 26 V — at 26 V / 0.05 Ω the cold corner is ${f(EX.iCold(26, 0.05), 1)} A, over I_max (${f(EX.iF(26, 0.05, vbr[0], rdyn8), 1)} A even at the 25 °C V_BR,min). Bare thresholds at 38 A: ${[24, 26, 35].map((v) => `${v} V ${f(Math.max(rMin[v], 0), 3)} Ω`).join(", ")}, negative ${f(EX.rExtMinNeg(24), 3)} Ω (0.258 with a 0.9 V forward drop) → IR-16 states 0.08 / 0.14 / 0.37 / 0.26 Ω (rounded up; the SMDJ7.0A-HRA of round 19 adds ≈ 0.043 Ω to each positive figure against the 8.5A). R_min 0.35 Ω is a 23 °C figure and a cold PTC is lower: VR-16 asks for the −40 °C minimum. If the OEM cannot state the 24 V figure, a 0.1 Ω pulse-rated 2512 in series per line (−0.3 % winding amplitude) or the unsuffixed MF-MSMF020 (I_max 80 A, slower) are the two routes (xcheck19 §1i). The rated diversion adds only 0.1–0.25 A here (it takes its current from the TVS)`);
    add("Sensing A.15", "Exciter PTC current vs I_max 40 A — load-dump-coincident fault (35 V)", `at 0.1 Ω external: ${f(EX.iCold(35, 0.1), 0)} A > 40 A · at the IR-16 minimum ${IR16[35]} Ω: ${f(iAt(35), 1)} A (bare thresholds: ${f(rMin[35], 3)} Ω at 38 A, ${f((35 - vbrCold) / 40 - rPtc - rdyn8, 3)} Ω at 40 A — round 18 printed the latter rounded DOWN to 0.29); with the TVS intact the PTC sees V_S − V_TVS ≈ ${f(35 - vCl(35, IR16[35]), 1)} V, under its 33 V V_max (round 18's "exceeded by 6 %" was wrong — the round-19 cross-check)`, "double event (IR-16 ≥ 0.37 Ω / IR-33)", "INFO",
      "the PTC still trips (faster than at 8 A); above I_max its survival is not warranted — it may fail open, which the FW-10 amplitude window reports as a resolver fault (fail-safe, not fail-dangerous). Accepted as a double event; the OEM allocation is IR-16 (0.37 Ω at 35 V since round 19, at the 5 % margin) and the PTC question VR-16");
    ...
    const s24 = solve0(24, IR16[24]), s35 = solve0(35, IR16[35]);   // round 20 (F207): at the IR-16 allocations — the 0 / 0.29 Ω of round 18 belonged to the 8.5 V class
    ...
    const vFin = vTh(35, IR16[35]) - vfDex, tau = (2.2 + rDex + 0.018) * cRail, qRev = cRail * vFin, i2t = s35.iDiv ** 2 * tau / 2, eDiode = vfDex * qRev + rDex * i2t, eRsx = 2.2 * i2t;
    ...
    judge("Sensing A.15", "Exciter back-drive with VEXD absent — rated diversion THROUGH RSX (DEXP/DEXN on the amplifier node, round 18)", `${f(s24.iDiv, 2)} A (24 V / ${IR16[24]} Ω) – ${f(s35.iDiv, 2)} A (35 V / ${IR16[35]} Ω) peak through RSX + DEX while the TVS holds the node at ${f(s24.v, 2)}–${f(s35.v, 2)} V (TVS ${f(s24.iTvs, 0)} / ${f(s35.iTvs, 0)} A, PTC total ${f(s24.iPtc, 1)} / ${f(s35.iPtc, 1)} A); τ = ${f(tau * 1e6, 0)} µs, ${f(qRev * 1e3, 2)} mC, ${f(eDiode * 1e3, 2)} mJ in the Schottky, ${f(eRsx * 1e3, 1)} mJ in RSX; rail ≈ ${f(vFin, 1)} V, ≤ ${f(vbr[1] + 40 * rdyn, 1)} V`,
...
## Findings log (F1–F36 rev A.3 campaign · F37–F46 rev A.4 · F47–F51 rev A.4.1 · F52–F57 rev A.4.2 · F58–F59 rev A.4.3 · F60–F62 rev A.5 docs audit · F63–F76 rev A.6 external review round 6 · F77–F89 rev A.7 review round 7 · F90–F97 rev A.8 review round 8 · F98–F105 its cross-check · F106–F113 rev A.9 review round 9 · F114–F119 its cross-check · F120–F122 rev A.10 schematic rechecks · F123–F134 rev A.11 system review · F135–F142 rev A.12 rechecks, pin freeze and production closure · F143–F158 rev A.13 round 14 · F159–F166 rev A.14 round 15 · F167–F173 rev A.15 round 16 (three rechecks of 32214be) · F174–F184 rev A.16 round 17 (gap closure) · F185–F189 rev A.16 round 17 (LV entry: test-B let-through, pulses 1/2a, FLVC, KL15) · F190–F198 rev A.17 round 18 (rechecks of 4425af9: diode pin numbers, the diversion through RSX, TVS/PTC coordination, RSX, acquisition-time firmware) · F199–F204 rev A.18 round 19 (rechecks of e315bf1: the 26 V endpoint, the ≥ 8 A trip-time law, the resolver time origin, the window criterion; the Marine sweep) · F205–F207 rev A.19 round 20 (rechecks of 00f6252: alternates and figure alignment) · F208 rev A.20 round 21 (rechecks of 551c405: the round-18 0.29 Ω in gate ㉘ and the QP-RX-04 fixture instruction, the last 8.5 V-era figures) · F209–F213 rev A.21 round 22 (rechecks of 0ff44d9: the §0.2 baseline; the layout handoff — footprint library and proof, 5-lead DPAK, thermal pads, 2-pad crystal) — all fixed; review cross-reference in [\`review-A6-disposition.md\`](review-A6-disposition.md), [\`review-A7-disposition.md\`](review-A7-disposition.md) and [\`review-A8-disposition.md\`](review-A8-disposition.md))
...
| F174 | MED | Round 17 (gap closure): the exciter back-drive with VEXD absent stayed OPEN because the charge into the 26.7 µF rail ran through the ALM2402 reverse diodes (pulsed use only, no envelope) | DEXP/DEXN Nexperia PMEG4050EP-Q (AEC-Q101, I_FSM 70 A, V_F 0.49 V at 5 A) from each protected node to VEXD carry the 4.9 A / 59 µs / 0.29 mC exponential (8.5 V class; ≈ 3.9 A / 60 µs / 0.23 mC since round 19); NCV4276C output abs max 40 V covers the back-fed rail — row PASS with a rated path |
...
| F199 | MED | Round 19 (A17-R01 md, rechecks of e315bf1): the round-18 PTC-current row was evaluated at 24 V only while IR-16 allowed 0.05 Ω up to the 26 V jump start — 41.6 A at 26 V / 0.05 Ω at the cold corner, over the PTC's 40 A; the 35 V minimum had been rounded DOWN to 0.29 Ω | the row runs every permitted voltage (16 / 24 / 26 / 35 V) from the shared model; the minima are computed at the verifier's own 5 % margin (38 A) and rounded UP — IR-16: 0.09 Ω at ≤ 26 V, 0.33 Ω at 35 V, 0.25 Ω negative at the 8.5 V class — superseded within the round by 0.08 / 0.14 / 0.37 / 0.26 Ω at the 7.0 V class (F203; bare 40 A thresholds stated beside them); QP-RX-04 step 2 runs 16 / 24 / 26 V; VR-16 asks for R_min at −40 °C |
| F200 | MED | Round 19 (A17-R02 md/html, all three rechecks): the ≥ 8 A single-fault TVS-energy PASS rested on "trip time falls at least as fast as I⁻² above 8 A" — the sheet gives one maximum (8 A / 20 ms) and a TYPICAL curve; nothing above 8 A is guaranteed | the row is a WARN with the assumption explicit (the 8 A max/typ ratio 3.8× carried up the typical curve: 0.5 J at 40 A; the full-20 ms sensitivity 4.3 / 8.9 J printed beside it); QP-RX-04 step 2 is a RELEASE CRITERION (trip ≤ 20 ms and ∫v·i ≤ 5.3 J at 0 / 0.09 / 0.5 Ω — 0 / 0.14 / 0.5 / 1.33 Ω since F203 — cold / hot / post-trip); VR-16 asks Bourns for the maximum envelope above 8 A vs temperature |
| F201 | **HIGH** | Round 19 (A17-R01 html/csv, firmware): the resolver ring anchored its time origin on the FIRST completion callback's own execution time (and again after every resync), so an over-budget first callback became the reference (80 µs late → every frame stamped 80 µs too new, 19.2° at 10 000 rpm / 4 pp, broken stays false), a constant 40–60 µs callback delay passed the 30 µs deadline forever, and a resync from equal DMA slots established count but not phase — the "anchor residual" round 18 had left as a target measurement | FW-35 rewritten: the origin is the SWG start instant on the STM timer (the SDADC blocks are triggered by the SWG period start), anchored by hal_swg_start() with a declared uncertainty (enable-write bracket + cal_swg_start_lat_us, T-42); the ring never anchors itself; every completion, including the first and every one after a resync, is judged against the absolute cadence; resync derives the block in progress from the clock and checks each DMA slot against it (a mismatch is lost); ambiguity → a synchronized producer restart (hal_sdadc_restart, ≤ cal_rslv_restart_max per key cycle, DTC counted); T-41 (error flags → lost) |
| F202 | LOW | Round 19 (self-found): the round-18 "unprotected on paper" window row printed 1–200 Ω at every voltage — its criterion OR-ed the energy test with an instantaneous-power test against the steady-state limit, so a 0.5 Ω fault at 167 W for 1.8 ms counted as unprotected | criterion corrected in the shared calculations/exciter-fault.mjs (energy where the PTC trips, steady-state power where it never does): 14–83 Ω at 24 V, 6–34 Ω at 16 V, 16–95 Ω at 26 V — the round-18 cross-check's figures; the model is one module now, run for Road (Sensing A.17) and Marine (marine-verify §7: the kit's 12 V rail leaves the TVS at 5.2–8.3 W by the uncapped 8.5 V-class formula — 2.0–2.5 W at the 7.0 V class after F203/F204 — the platform's worst case → QP-MA-11, the Marine kit-loom requirement ≥ 0.25 Ω interim, 0.27 Ω at the 7.0 V class) |
| F203 | MED | Round 19 (Marine sweep + Opus cross-check xcheck19): with the 8.5 V-class exciter TVS the sustained-short trickle after the PTC trips (V_BR·P_d/(V_S − V_BR), capped at V_BR·I_trip) is 3.8–4.3 W at a parked 12.0–12.6 V battery and at the Marine kit's 11.4–12 V rail, and on the coupled island the junction still reaches 148–171 °C because the coupling benefit scales with (V_S − V_BR)/V_BR, which only the class sets | TVSEP/TVSEN → **SMDJ7.0A-HRA** (same SMC pad, same AEC-Q101 series, same price): trickle 1.7–2.5 W uncapped, junction ≤ 132 °C coupled at T_t 125 °C; every dark condition kept with ≥ 2.4 V margin (rail present, VMID stuck high); leakage ≤ 1.4 % peak dip at 125 °C on the ×10/55 K bound; +0.043 Ω on each harness allocation (IR-16 0.08 / 0.14 / 0.37 / 0.26 Ω; the OEM's on-fail routes: a 0.1 Ω pulse-rated 2512 per line, or the 80 A unsuffixed MSMF020); the coupled-island row added; dfm.md specifies the PAIR's transfers |
| F204 | LOW | Round 19 (cross-check corrections of round-18 figures): the trickle formula was uncapped (5.2 W printed at 12 V where a tripped PPTC cannot pass more than I_trip: 4.2 W); the 35 V row said "V_max 33 V exceeded by 6 %" although the PTC sees V_S − V_TVS ≈ 26 V while the TVS is intact; I_R of the 8.5A is 20 µA (not 10); the plain SMDJ sheet's Z_th Fig. 6 (1 ms–1000 s, typical) already answers most of VR-17; R_PTC and P_d are 23 °C figures (cold R lower, cold P_d ≈ ×1.6 assumed) | figures corrected in the shared module and the rows; VR-17 narrowed (confirm Fig. 6 for the HRA part + the single-pulse T_J limit 10 ms–10 s); VR-16 (c)/(e) carry the cold R and the tripped P_d vs voltage |
| F205 | MED | Round 20 (A18-D01 html / A18-R01 md, rechecks of 00f6252): after the round-19 class change the exciter TVS alternate still read "SMDJ8.5A (same electricals …)" — a different protection configuration (9.44–10.40 V, 3.8–4.3 W parked trickle) that cannot inherit the 7.0 V qualification | alternate → the plain SMDJ7.0A (same electricals as the -HRA, no AEC-Q101 statement, proto only), the 8.5 V class named as excluded from the A.18 terminal-fault qualification; ERC lock: the exciter TVS alternate must name the primary's voltage class (per SKU) |
| F206 | MED | Round 20 (A18-R02 md): the 4XX discharge-resistor row (four 220 Ω, SQP10-220RJB15) carried the 8XX alternate "Yageo SQP10AJB-470R" — four 470 Ω would take 3.1 s to 60 V (3.5 s at +5 % / +10 %) against the 2 s target; the 220 Ω primary takes 1.5 s | alternate → Yageo SQP10AJB-220R under the same no-flame condition (VR-28); ERC lock: a discharge-resistor alternate must carry the row's value (the 8XX row's value read from its MPN) |
| F207 | LOW | Round 20 (A18-N01, both reviews): the target checklist rows T-05/T-06 still named TI_FW_ID 0x0A0F0012 (the header has 0x0A0F0013) — an avoidable no-arm at the validation station; and 8.5 V-era figures survived the class change in the card comments (clamp 10.4–11.5 V, 4.8 A / 0.28 mC), the README gate, the design-basis §11r values, the F199 row, QP-RX-04/05 predictions and the back-drive row's evaluation conditions (0 / 0.29 Ω → PTC totals over 40 A printed beside a PASS) | every figure re-derived from the shared model at the IR-16 allocations (≈ 3.9 A / 60 µs / 0.23 mC, node ≈ 9.1 V, clamp ≤ 9.1 V at 40 A) with the 8.5 V-class values marked historical; T-05/T-06 → 0x0A0F0013 |
| F208 | LOW | Round 21 (A19-N01, all three rechecks of 551c405): README gate ㉘ still quoted IR-16 ≥ 0.29 Ω for the 35 V double event where IR-16 states 0.37 Ω (42.6 A against 37.9 A at the cold corner — a fixture set from the gate text would exceed the PTC's 40 A I_max); broader than reported: the same round-18 figure sat in QP-RX-04's own step-3 instruction, and 8.5 V-era figures survived in the QP's "Closes" row name (0.05 Ω), its step-2b Marine cross-reference (5.2 / 8.3 W — the uncapped formula of round 18), its post-trip pass criterion (0.6 / 0.3 / 1.5–3.8 W) and QP-RX-05's rail figure (10.5 V), VR-33's envelope (4.8 A / 0.28 mC / ≤ 11 V), the README gate's and IR-42's trickle figure and the gate's sweep resistances (1.5–4 W; 0 / 0.09 / 0.5 Ω), the card comment, the RSX BOM description and the PMEG4050EP-Q duty row of EXTRACTED-PARAMS (4.8–4.9 A / 1.6 mJ / 0.29 mC) and the round-18/19 narratives | every live figure reads the 7.0 V-class value from the shared model (0.37 Ω; 0.9–1.7 W at 12.6–16 V, 0.45 / 0.26 W at 24 / 35 V; 2.0 / 2.5 W on the Marine kit rail, kit loom ≥ 0.27 Ω; ≈ 3.9 A / 60 µs / 0.23 mC, rail ≈ 8.6 V, ≈ 1.0 mJ in RSX; the ≥ 8 A sweep at 0 / 0.14 / 0.5 / 1.33 Ω) with the 8.5 V-class value marked historical beside it; no circuit, BOM-total or firmware change |
| F209 | LOW | Round 22 (A20-N01, review 1 of 0ff44d9): the qualification plan's general DUT table (§0.2) still defined the hardware as A.15+ with SMDJ8.5A-HRA, the production image as TI_FW_ID 0x0A0F0011 and the validation record for that ID, while §7 and the BOM carry the SMDJ7.0A-HRA and the header 0x0A0F0013; the same round-17 ID sat in the QP-EOL pass criterion, the validation-record specification and the closes-list, in firmware/README items 22 and 29 and in firmware-contract §10c; the layout-facts sweep also found the README's DFM table still describing the superseded 10 × 27 k bleeder (the released board has 12 × 22 k, 6s×2p), the MCU manifest saying 158 open balls beside its own 159, VR-17 still asking about the SMCJ8.5A, the RFS4 fault-hold row and dfm §4 fillet rule still on the ESR03 | §0.2 → rev A.20+ hardware (SMDJ7.0A-HRA, ESR18EZPF2R20, the A.15–A.17 build named as a different protection configuration), FW 0x0A0F0013, VAL for 0x0A0F0013 measured on the image flashed; every other stale ID marked as history; README bleeder row, manifest count, VR-17 (SMDJ7.0A-HRA, the F204 narrowing), the FS1B row limit (ESR18: 0.41 W at 85 °C, 2.0 W for 5 s) and the dfm fillet rule (ESR Rev.012 Fig. 2/4 knees: RFS4 ≤ 137 °C at 0.30 W) corrected; QP §15 item 6 bullets given their resolutions |
| F210 | **HIGH** | Round 22 (the user's question — why the schematic could not go to layout without worry; found by inspecting the handoff, not by the reviewers, who excluded PCB layout): every KiCad symbol carried a bare package code (R0603, MAPBGA289, …) and the sets shipped no footprint library — the KiCad README said so — so Update PCB from Schematic could not run for any of the 686 components; the tscircuit footprints are generic placeholders (0.9 × 1.4 mm pads for every IC) | calculations/footprints.mjs binds every code to one footprint in a shipped project library, kicad/traction/traction.pretty: standard patterns copied VERBATIM from the KiCad 10.0.6 libraries (names kept, sha256 in SOURCES.json, CC-BY-SA 4.0 with the KiCad exception) and the rest drawn from the archived datasheets pad by pad (traction.pretty/MANIFEST.md — EconoDUAL 3 aux pins and terminals, UCC14141 DWN-36, AMC1311B DWV, VOW3120, VGT12EEM, Samtec IPL1 ×3, TE 770669-1, Faratronic can, Bel 2410 fuse, TDK ACT45B, Yageo SQP10, onsemi SMA flat-lead, Kyocera 2-pad crystal); fp-lib-table in the KiCad 9/10 set and both KiCad 5 sets and their zips; F2 = traction:<name> on every on-board symbol, the three off-board LEM sensors excluded from the board on purpose; kicad5-gen REFUSES a part that resolves to nothing; kicad-sch-verify now judges footprint_link_issues (0 allowed except off-board) and proves that every symbol pin number has a pad of that number; docs/layout-handoff.md is the layout engineer's entry point (stack, copper, creepage basis, rules by block, open decisions, first-article access) |
| F211 | MED | Round 22 (the new pad-per-pin proof): the five NCV4276C regulators (U5LB/U5LC/UGDL DT50, ULDOEX DTADJ, ULDO15 DSADJ) were coded DPAK / D2PAK as 3-lead lands, while the family exists only as 5-lead DPAK/D2PAK (DS p.1: pin 2 INH, tab = pin 3 GND); a 3-lead land would have left INH and OUT without pads under a symbol that uses them | codes bound to TO-252-5_TabPin3 / TO-263-5_TabPin3; the pin audit confirms all five symbols against onsemi's 5-lead pinout |
| F212 | MED | Round 22 (pin audit, Sonnet, 31 part groups against their datasheets): the ALM2402 PWP exposed pad and the TPS55340 RTE PowerPAD had no symbol pin, so a real land would have left them without a net — TI: the ALM2402 pad goes to ground and to nothing else, the TPS55340 PowerPAD MUST be soldered to AGND; 30 of 31 groups match pin for pin (the FS26's 48 + EP now confirmed, the VGT12EEM dots, the LFPAK56 tab as pin 5, the TO-247-4 lead order, the ACT45B winding pairs) | UEXD pin 15 = PAD on AGND and UB15 pin 17 = PAD on DGND (the device AGND net) in the sources, the ERC lock updated (15 pins, pad on AGND); the KiCad lands net their pads 15 / 17 — proved by the verifier; the SOT-223 QLVS whose datasheet prints no pin numbers is resolved by the land's TO-261 orientation (tab right: top lead 1 = G, middle 2 = D = tab, bottom 3 = S — matches the symbol) |
| F213 | LOW | Round 22 (pin audit): the MCU crystal Y1 (Kyocera CX3225GA) is a two-terminal part, and the generic 4-pad 3225 pattern would have put it on two of four pads with two floating | X3225 → the Kyocera 2-pad land drawn from the sheet's recommended pattern (Kyocera_CX3225GA_2Pad) |
